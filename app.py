...
def apptListdr(username):
	cur = mysql.connection.cursor()
	res2 = cur.execute("SELECT a.patient_id, p.patient_name, a.appt_date, a.time  FROM appointments a, patients p, doctors d WHERE a.patient_id = p.patient_id AND d.doctor_id = a.doctor_id AND d.doc_username = %s", [username])
	rv = cur.fetchall()
